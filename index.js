  ...
  if(typeof socket.handshake.query.nickname != "undefined"){     
	...
	socket.on('private message', function(data){  
		let user = online.find(e=>e.name == data.to)  
  		io.to(user.socket.id).emit('chat message', { name: data.from, msg:"private: " + data.msg}); 
	});
	...
	socket.on('room', function(data) {
		...
		socket.to('room-one').emit('chat message',{name: data.name, msg:"has entered the room"}); 
	});
